  ...
  return (
    <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 space-y-6 shadow-xl text-slate-900">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          <span className="text-xs text-emerald-700 font-mono font-extrabold uppercase">{t('resultsSummary', 'RESULTS SUMMARY')}</span>
          <h3 className="text-2xl font-extrabold text-slate-900 mt-0.5">{t('yourTripEstimateTitle', 'YOUR TRIP ESTIMATE 🎯')}</h3>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-bold">
          <ShieldCheck className="w-4 h-4 text-emerald-600" /> {t('highAccuracyModel', 'High Accuracy Model')}
        </div>
      </div>

      <div className="p-4 rounded-2xl bg-amber-50/70 border border-amber-200 text-center space-y-1">
        <span className="text-slate-600 text-xs font-bold">{t('tripEstimatedUse', 'Your trip is estimated to use:')}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
        <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200 space-y-1">
          ...
          <span className="text-slate-600 text-xs font-bold block">{t('estFuelCost', 'Estimated fuel cost')}</span>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-teal-50/70 border border-teal-200 space-y-1">
          ...
          <span className="text-slate-600 text-xs font-bold block">{t('estCo2e', 'Estimated CO₂e')}</span>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-blue-50/70 border border-blue-200 space-y-1">
          ...
          <span className="text-slate-600 text-xs font-bold block">{t('estTravelTime', 'Estimated travel time')}</span>
        </div>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md">
        <div className="space-y-0.5 text-center sm:text-left">
          <span className="text-white font-extrabold text-sm block">{t('wantBetterOption', 'Want to see if GreenFleet can find a better option?')}</span>
          <span className="text-emerald-100 text-xs">{t('compareAltRoutes', 'Compare alternative routes and prioritize cost, emissions or time.')}</span>
        </div>
        ...
        >
          <span>{t('findBetterRouteBtn', 'Find a Better Route →')}</span>
        </button>
      </div>
    </div>
  );
