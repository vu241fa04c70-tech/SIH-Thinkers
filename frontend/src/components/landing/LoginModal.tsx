    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 w-full max-w-md space-y-6 relative shadow-2xl">
        ...
        <div className="space-y-2 text-center">
          ...
          <h3 className="text-xl font-extrabold text-white tracking-tight">Login to GreenFleet AI</h3>
          <p className="text-xs text-slate-400">Access Quantum QUBO Optimizer & Fleet Analytics</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          <div>
            <label className="text-slate-400 font-semibold block mb-1">Select Role</label>
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: 'operator', label: 'Fleet Operator' },
                { id: 'manager', label: 'Logistics Mgr' },
                { id: 'analyst', label: 'Eco Analyst' }
              ].map(r => (
                <button
                  key={r.id}
                  type="button"
          ...
          <div>
            <label className="text-slate-400 font-semibold block mb-1">Work Email</label>
        ...
        <div className="pt-2 border-t border-slate-800/80 text-center text-[11px] text-slate-500 flex items-center justify-center gap-1.5">
          <ShieldCheck className="w-4 h-4 text-emerald-400" /> Secure Enterprise Single Sign-On (SSO)
        </div>
      </div>
    </div>
  );
