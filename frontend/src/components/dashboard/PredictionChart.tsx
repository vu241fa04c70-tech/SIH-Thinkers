    ...
    <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-md space-y-4 text-slate-900">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">GreenFleet's routes use less fuel than normal routes.</p>
        </div>
        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-800 border border-emerald-200">
          -21.3% Average Savings
        </span>
      </div>
      ...
      <div className="h-72 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <Legend />
            <Area type="monotone" dataKey="baseline" name="Normal Route" stroke="#f59e0b" fillOpacity={1} fill="url(#colorBaseline)" />
            <Area type="monotone" dataKey="optimized" name="GreenFleet Route" stroke="#10b981" fillOpacity={1} fill="url(#colorOptimized)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
